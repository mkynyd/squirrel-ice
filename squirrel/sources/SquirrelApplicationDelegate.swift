...
final class SquirrelApplicationDelegate: NSObject, NSApplicationDelegate, SPUStandardUserDriverDelegate, UNUserNotificationCenterDelegate {
  ...
  func openSettings() {
    WuSongConfigManager.shared.reload()
    WuSongSettingsWindowController.shared.show()
  }
  ...
  func setupRime() {
    ...
    let version = Bundle.main.object(forInfoDictionaryKey: kCFBundleVersionKey as String) as? String ?? "0"
    needsFullDeploy = WuSongConfigManager.shared.deployBundledConfigIfNeeded(currentVersion: version)
  ...
  func addObservers() {
    ...
    notifCenter.addObserver(forName: .init(WuSongConfigManager.configChangedNotificationName), object: nil, queue: nil, using: rimeNeedsReload)
  }
